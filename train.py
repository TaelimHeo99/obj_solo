...
Gradient Accumulation:
  --accumulate N   # accumulate N mini-batches before optimizer.step()
  If --accumulate <= 0, it falls back to the original auto-accumulation logic (nbs=64).
"""
...
from models.yolo import Detect  # for type hints if needed
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # ---- Apply global SNN params (no-op for YAML models) ----
    set_global_snn(
        thresh=opt.snn_thresh,
        subthresh=opt.snn_subthresh,
        LIF_tau=opt.snn_tau,
        mem_init=opt.snn_mem_init,
    ...
    # Scale weight decay to effective batch size
    hyp["weight_decay"] *= batch_size * accumulate_base / nbs
    ...
    if RANK in {-1, 0}:
        ...
        if not resume:
            ...
            # If fp16 causes instability with SNN/BNS, you can replace with model.float()
            model.half().float()
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if RANK in {-1, 0}:
            ...
            if hasattr(m_par, "reset_states"):
                m_par.reset_states()
            ema_par = ema.ema if ema else None
            ...
            if (not nosave) or (final_epoch and not evolve):
                ckpt = {
                    ...
                    "model": deepcopy(m_par).half(),
                    "ema": deepcopy(ema_par).half() if ema_par is not None else None,
...
if __name__ == "__main__":
    opt = parse_opt()
    # W&B sane defaults
    if not hasattr(opt, "entity"):
        opt.entity = None
    if not hasattr(opt, "wb_project"):
        opt.wb_project = None
    if not hasattr(opt, "disable_wandb"):
        opt.disable_wandb = False
    main(opt)
