...
class Loggers:
    ...
    def __init__(self, save_dir=None, weights=None, opt=None, hyp=None, logger=None, include=LOGGERS):
        ...
        # W&B (robust guard: disable if run handle is missing)
        if wandb and "wandb" in self.include:
            self.opt.hyp = self.hyp  # add hyperparameters
            _wl = None
            try:
                _wl = WandbLogger(self.opt)
            except Exception as e:
                _wl = None
                LOGGER.info(f"[W&B] initialization skipped: {e}")
            # if wandb_run didn't initialize (disabled or error), null out to avoid callbacks touching it
            self.wandb = _wl if (_wl and getattr(_wl, "wandb_run", None)) else None
        else:
            self.wandb = None
